#!/usr/bin/env bash
# automerge.sh <worktree-name> [<base-ref>]: when the land base moved and the dry run is clean, merge its remote ref into a detached
...
BASE=${2:-$DEFAULT}; BASE=${BASE#origin/}; REMOTE_BASE="origin/$BASE"
git -C "$ROOT" fetch -q origin
H=$(git -C "$WT" rev-parse HEAD); B=$(git -C "$ROOT" rev-parse "$REMOTE_BASE")
[ -z "$(git -C "$WT" status --porcelain)" ] || { echo "$WT is not clean; refusing" >&2; exit 1; }
if git -C "$ROOT" merge-base --is-ancestor "$B" "$H"; then echo "nothing to merge: $REMOTE_BASE (${B:0:7}) is already in ${H:0:7}"; exit 0; fi
if git -C "$ROOT" merge-base --is-ancestor "$H" "$B"; then echo "${H:0:7} is already inside $REMOTE_BASE (${B:0:7}): the branch has landed or is behind with nothing of its own; fast-forward the worktree by hand if you still need it"; exit 0; fi
if ! AUTO=$(git -C "$ROOT" merge-tree --write-tree "$H" "$B" 2>/dev/null); then
  echo "conflicts merging $REMOTE_BASE (${B:0:7}) into ${H:0:7}; this is a merge round:" >&2
...
git -C "$M" merge --no-edit -m "Merge $REMOTE_BASE (${B:0:7}) into $(git -C "$WT" branch --show-current)
...
echo "automatic merge ${SHA:0:7} of $REMOTE_BASE (${B:0:7}) into ${H:0:7} at $M (tree == merge-tree). Capture the suite on $1-merge, then: automerge.sh --adopt $1"
