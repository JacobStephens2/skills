...
6. **Converge after correction.** Re-run the mechanical gate. Then run `scripts/review.sh` with the ledger path. The correction verification checks the recorded blockers and may block on a concrete regression introduced after the ledger's `reviewed-head`. A pre-existing architectural alternative discovered late is a suggestion. Update the same ledger; it remains the approval bar through correction.
