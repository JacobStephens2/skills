...
5. Find the **suite command** that covers the tickets' code, from the environment (`package.json` `test`/`check`, a Makefile target, CI, a test README), what a worktree needs to run it (a `.venv`, `node_modules`), and its exclusive shared resources (fixed ports, named simulators or emulators, databases). A repo with more than one suite gets the suite the tickets change. Write all three in the table.
